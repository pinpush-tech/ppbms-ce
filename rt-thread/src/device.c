                          ...
                          rt_size_t   size)
{
    ...
    {
        // @changed
        if (dev->tx_starting_pp != RT_NULL)
        {
            dev->tx_starting_pp(dev);
        }
        rt_size_t res_size = device_write(dev, pos, buffer, size);
        if (dev->tx_complete_pp != RT_NULL)
        {
            dev->tx_complete_pp(dev);
        }
        return res_size;
    }
...
// @changed
rt_err_t rt_device_set_tx_starting_pp(rt_device_t dev,
                                   rt_err_t (*tx_done)(rt_device_t dev))
{
    /* parameter check */
    RT_ASSERT(dev != RT_NULL);
    ...
    dev->tx_starting_pp = tx_done;

    return RT_EOK;
}
RTM_EXPORT(rt_device_set_tx_starting_pp);
rt_err_t rt_device_set_tx_complete_pp(rt_device_t dev,
                                   rt_err_t (*tx_done)(rt_device_t dev))
{
    /* parameter check */
    RT_ASSERT(dev != RT_NULL);
    ...
    dev->tx_complete_pp = tx_done;

    return RT_EOK;
}
RTM_EXPORT(rt_device_set_tx_complete_pp);
