...
cJSON* csonEncodeObject(void *obj, CsonModel *model, int modelSize)
{
    ...
    for (short i = 0; i < modelSize; i++)
    {
        ...
        case CSON_TYPE_LONG:
            ...
            break;
        case CSON_TYPE_UCHAR:
            csonEncodeNumber(root, model[i].key, *(unsigned char *)((int)obj + model[i].offset));
            break;
        case CSON_TYPE_USHORT:
            csonEncodeNumber(root, model[i].key, *(unsigned short *)((int)obj + model[i].offset));
            break;
        case CSON_TYPE_UINT:
            csonEncodeNumber(root, model[i].key, *(unsigned int *)((int)obj + model[i].offset));
            break;
        case CSON_TYPE_ULONG:
            csonEncodeNumber(root, model[i].key, *(unsigned long *)((int)obj + model[i].offset));
            break;
        case CSON_TYPE_FLOAT:
        ...
        case CSON_TYPE_STRING:
            ...
            {
                csonEncodeString(root, model[i].key, (char *)((int)obj + model[i].offset));
            }
            break;
        case CSON_TYPE_LIST:
